...
async function grade(s, studentImage) {
  ...
  if (!s.answerText && !s.answerImage)
    text += "\n모범답안이 제공되지 않았습니다. 채점하기 전에, 당신이 먼저 이 문제를 처음부터 끝까지 직접 풀어 정확한 정답을 구하세요. 학생의 답안을 보기 전에 당신의 정답을 먼저 정하고, 그것을 기준으로 채점하세요. 학생이 쓴 답에 절대 끌려가지 마세요.\n";
  ...
  text += "\n위 마지막 이미지가 학생의 답안입니다. 다음 기준으로 문제 유형을 먼저 판단하고 채점하세요.\n\n" +
    ...
    "■ 그 외 일반 계산·증명·서술형 문제라면(부등식·방정식·확률 등 포함): type을 \"calc\"로 하고, 풀이 단계와 최종 답이 맞는지를 단계별(features의 name을 '1단계: ...' 식으로)로 채점하세요. features에 반드시 '최종 답' 항목을 하나 넣어, 학생의 최종 답이 정답과 일치하는지 ok로 표시하세요.\n\n" +
    ...
    "채점 엄격성(매우 중요): \n" +
    "- 먼저 정답(모범답안 또는 당신이 직접 구한 답)을 확정한 뒤, 학생의 최종 답이 그것과 실제로 일치하는지 비교하세요.\n" +
    "- 학생의 최종 답이 정답과 다르면 절대 만점(100점)을 주지 마세요. 부등식이면 부등호 방향과 경계값, 구간 표기까지 정확히 일치해야 정답입니다.\n" +
    "- 풀이 과정이 그럴듯해 보여도 최종 답이 틀리면 반드시 틀린 항목으로 표시하세요. 서로 다른 답은 둘 다 정답일 수 없습니다.\n" +
